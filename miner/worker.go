	...
	for {
		...
		case ev := <-self.txCh:
			...
			if atomic.LoadInt32(&self.mining) == 0 {
				...
				feeCapacity := state.GetXRC21FeeCapacityFromState(self.current.state)
...
func (self *worker) commitNewWork() {
	...
	feeCapacity := state.GetXRC21FeeCapacityFromStateWithCache(parent.Root(), work.state)
...
func (env *Work) commitTransactions(mux *event.TypeMux, balanceFee map[common.Address]*big.Int, txs *types.TransactionsByPriceAndNonce, specialTxs types.Transactions, bc *core.BlockChain, coinbase common.Address) {
	...
	for _, tx := range specialTxs {
		...
		if tokenFeeUsed {
			...
			if env.header.Number.Cmp(common.TIPXRC21Fee) > 0 {
	...
	state.UpdateXRC21Fee(env.state, balanceUpdated, totalFeeUsed)
